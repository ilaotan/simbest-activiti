...
public interface CustomTaskMapper {
    ...
    @Select("SELECT * FROM ( SELECT DISTINCT s.* FROM act_business_status s,act_task_assigne a WHERE s.processDefinitionId=a.processDefinitionId AND s.processInstanceId =a.processInstanceId AND s.iscg=0 AND a.completeTime IS NOT NULL AND (a.owner=#{uniqueCode} OR a.assignee=#{uniqueCode}) ORDER BY s.startTime DESC ) tbl LIMIT #{pageindex},#{pagesize}")
